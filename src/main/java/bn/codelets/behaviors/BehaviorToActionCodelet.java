...
public class BehaviorToActionCodelet extends Codelet {
    ...
    private int creatureBasicSpeed=3;
    private double reachDistance = 50;
    ...
    public void proc() {
        if (chosenBehaviorMO != null) {
            ...
            if(chosenBehavior.equals(lastBehavior) && !lastBehavior.equals("EXPLORE_COMPETENCE")){
                return;
            }
...
//              System.out.println("Removendo a apple: " + closestApple.getName() + "Known Apples list: " + knownApples);
          }
          closestApple = null;
        }
     }
    
    private void exploreProc() {
        if(isCreatureInLastTarget() || !lastBehavior.equals("EXPLORE_COMPETENCE")) {
            Random random = new Random();
            int maxX = 800;
            int maxY = 600;
            positionX = (double) random.nextInt(maxX + 1);
            positionY = (double) random.nextInt(maxY + 1);
            ...
            message.add(Idea.createIdea("X",(int)positionX, Idea.guessType("Property",1)));
            message.add(Idea.createIdea("Y",(int)positionY, Idea.guessType("Property",1)));
